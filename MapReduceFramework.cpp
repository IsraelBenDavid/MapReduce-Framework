...
typedef struct Job {
    pthread_t *workers;
    Barrier *barrier;
    sem_t map_mutex, reduce_mutex, emit2_mutex, emit3_mutex;
...
void emit2 (K2 *key, V2 *value, void *context)
{
  ...
  thread_context->intermediate_vec.push_back ({key, value}); // todo: static cast?
...
void emit3 (K3 *key, V3 *value, void *context)
{
  ...
  thread_context->job->outputVec->push_back ({key, value}); // todo: static cast?
...
{
  thread_context->job->job_state.stage = MAP_STAGE;
  while (thread_context->job->atomic_counter < thread_context->job->inputVec->size ())
    {
      sem_wait (&thread_context->job->map_mutex);
      auto old_value = thread_context->job->atomic_counter.load ();
      thread_context->job->atomic_counter++;
      sem_post (&thread_context->job->map_mutex);
      if (old_value < thread_context->job->inputVec->size ())
        {
          thread_context->intermediate_vec.clear ();
          InputPair pair = thread_context->job->inputVec->at (old_value);
          // call map
          thread_context->job->client->map (pair.first, pair.second, thread_context);
          thread_context->job->intermediate_vecs.push_back (thread_context->intermediate_vec);
        }
    }

  thread_context->job->job_state.percentage = 30;
}
...
void sort_phase (ThreadContext *thread_context)
{
  ...
  thread_context->job->job_state.percentage = 60;
...
{
  thread_context->job->job_state.stage = SHUFFLE_STAGE;
  ...
  thread_context->job->job_state.percentage = 80;
}
...
{
  thread_context->job->job_state.stage = REDUCE_STAGE;
  while ((thread_context->job->atomic_counter.load()>>16 & (0x7fff))
         < thread_context->job->shuffled_intermediate_vecs.size ())
    {
      sem_wait (&(thread_context->job->reduce_mutex));
      auto old_value = thread_context->job->atomic_counter.load()>>16 & (0x7fff);
                      ...
                      int multiThreadLevel)
{
  ...
  job_context->job->map_counter = 0;
  job_context->job->reduce_counter = 0;
...
{
  *state = ((JobContext *) job)->job->job_state;
}
